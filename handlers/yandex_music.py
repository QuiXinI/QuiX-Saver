...
YANDEX_MUSIC_DOMAINS = ("music.yandex.",)

@app.on_message(filters.text & filters.private & (filters.regex(r"(?i).*\b(" + "|".join(YANDEX_MUSIC_DOMAINS) + r")\b.*")))
@check_blacklist
async def yandex_music_handler(_, message: Message, url_override: str = None, user_id_override: int = None):
    """Handles Yandex.Music links."""
    ...
    try:
        ydl = get_ydl({'cookiesfrombrowser': ('firefox',), 'extract_flat': 'in_playlist'})
        info = await app.loop.run_in_executor(None, lambda: ydl.extract_info(url, download=False))
    except Exception as e:
        logger.error(f"Error fetching Yandex.Music info for {url}: {e}")
        await status_message.edit_text("❌ Не удалось получить информацию. Убедитесь, что ссылка корректна и доступна.")
        return
    ...
    author = info.get('artist', 'Неизвестный исполнитель')
